...
#define TRAIL_MAX         7
#define TAP_RADIUS_PX     RADAR_TAP_RADIUS    // generous finger-tap catch radius (picks the nearest glyph within it)
#define FLOW_MAX          700
...
static void grid_draw_cb(lv_event_t *e) {
    ...
    ad.color = s_cRing;
    ad.width = 2;
    const lv_coord_t rr[4] = { RADAR_RING0, RADAR_RING1, RADAR_RING2, RADAR_R_OUTER_PX };
    ...
    ll.width = 2;
    ll.opa = 41;
    lv_point_t h1 = { (lv_coord_t)(s_cx - RADAR_CROSSHAIR_HALF), s_cy }, h2 = { (lv_coord_t)(s_cx + RADAR_CROSSHAIR_HALF), s_cy };
    lv_point_t v1 = { s_cx, (lv_coord_t)(s_cy - RADAR_CROSSHAIR_HALF) }, v2 = { s_cx, (lv_coord_t)(s_cy + RADAR_CROSSHAIR_HALF) };
...
static inline lv_area_t glyph_bbox(lv_point_t p) {
    lv_area_t a;
    if (dragon()) { a.x1 = p.x - RADAR_GLYPH_PAD_DRG; a.y1 = p.y - RADAR_GLYPH_PAD_DRG;
                    a.x2 = p.x + RADAR_GLYPH_PAD_DRG; a.y2 = p.y + RADAR_GLYPH_PAD_DRG; }
    else          { a.x1 = p.x - RADAR_GLYPH_PAD;     a.y1 = p.y - RADAR_GLYPH_PAD;
                    a.x2 = p.x + RADAR_GLYPH_PAD_R;   a.y2 = p.y + RADAR_GLYPH_PAD; }
    return a;
}
...
void init(void *lv_parent) {
    ...
    s_rose[0] = make_label(parent, "N", &UI_FONT_ROSE_BIG,   COL_INK,  LV_ALIGN_TOP_MID,                       0,  RADAR_ROSE_PAD);
    s_rose[1] = make_label(parent, "S", &UI_FONT_ROSE_SMALL, COL_SOFT, LV_ALIGN_BOTTOM_MID,                    0, -RADAR_ROSE_PAD);
    s_rose[2] = make_label(parent, "E", &UI_FONT_ROSE_SMALL, COL_SOFT, LV_ALIGN_RIGHT_MID,  -RADAR_ROSE_PAD,                  0);
    s_rose[3] = make_label(parent, "W", &UI_FONT_ROSE_SMALL, COL_SOFT, LV_ALIGN_LEFT_MID,    RADAR_ROSE_PAD,                  0);

    char rng[16];
    snprintf(rng, sizeof(rng), "%.0f km", (double)RANGE_KM_DEFAULT);
    s_rangeLbl = make_label(parent, rng, &UI_FONT_SMALL, COL_GREEN, LV_ALIGN_CENTER, RADAR_RANGELBL_X, RADAR_RANGELBL_Y);
